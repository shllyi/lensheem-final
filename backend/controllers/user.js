...
//create admin
 const createAdmin = async (req, res) => {
    const { name, email, password, title, addressline, town, phone } = req.body;
    ...
    try {
        ...
        // Split name into first and last name for customer table
        ...
        await connection.promise().execute(
            'INSERT INTO customer (user_id, fname, lname, title, addressline, town, phone, image_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            [userId, firstName, lastName, title || '', addressline || '', town || '', phone || '', imagePath]
        );
...
// Reset all users to active state (fix for current issue)
const resetAllUsersToActive = async (req, res) => {
  try {
    const sql = `
      UPDATE users
      SET deleted_at = NULL
    ...
    return res.status(200).json({
      success: true,
      message: `Reset ${result.affectedRows} users to active state`,
...
const getAllUsersIncludingDeleted = async (req, res) => {
  try {
    const sql = `
      SELECT 
        u.id as user_id,
      ...
      FROM users u
      LEFT JOIN customer c ON u.id = c.user_id
      ORDER BY u.deleted_at ASC, u.id DESC
    `;
